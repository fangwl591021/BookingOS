...
window.addEventListener("load",async()=>{try{if(!shouldRunLiff)return;await initLiff();if(liff.isLoggedIn())await login();else setStatus("請按 LINE 登入 / 註冊");}catch(error){setStatus("Open from LINE App member link");}});
