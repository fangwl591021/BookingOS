...
function isBusinessOpenOnDate(businessHours = {}, dateText = "") {
  ...
  const weekday = weekdayLabelForDate(dateText);
...
function renderMerchantOnboardingPage(data = { store, services: [], staffMembers: [], resourceTypes: [], businessHours: {} }) {
  ...
  const access = data.store?.access || data.store?.tenantAccess || evaluateTenantAccess(data.store || {});
  const readOnly = !(access.operationalStatus === "active" || access.operationalStatus === "trial");
  const readOnlyMessage = access.operationalStatus === "grace" || access.operationalStatus === "expired" ? "目前方案已到期，設定資料僅供查看；續約後可繼續修改。" : "目前店家狀態僅供查看，請聯絡平台處理。";
  const readOnlyNotice = readOnly ? `<section class="panel" style="margin-bottom:12px;border-color:#f1d58a;background:#fffaf0"><h2>設定唯讀</h2><p class="muted">${escapeHtmlValue(readOnlyMessage)}</p></section>` : "";
  const onboardingActions = readOnly ? `<div class="onboard-actions"><button class="btn" type="button" data-copy-onboarding-url="${escapeAttrValue(publicPath)}">複製公開網址</button><a class="btn" href="${escapeAttrValue(publicPath)}" target="_blank">預覽客戶端</a></div>` : `<div class="onboard-actions"><button class="btn" type="button" data-test-booking="1">建立測試預約</button><button class="btn primary" type="button" data-open-booking="1">開放正式預約</button><button class="btn" type="button" data-close-booking="1">暫停正式預約</button><button class="btn" type="button" data-copy-onboarding-url="${escapeAttrValue(publicPath)}">複製公開網址</button><a class="btn" href="${escapeAttrValue(publicPath)}" target="_blank">預覽客戶端</a></div>`;
  const readOnlyScript = readOnly ? `document.querySelectorAll("form input,form textarea,form select,form button,[data-template],[data-test-booking],[data-open-booking],[data-close-booking]").forEach((el)=>{el.disabled=true;});` : "";
...
function renderPlatformOperationsPage(platform = { tenants: [], admins: [] }, currentData = { store }, env = {}, requestOrUrl = null) {
  ...
  const tenantRows = tenants.map((tenant) => {
    ...
    return `<details class="tenant-card" data-search="${escapeAttrValue([tenant.name, tenant.id, tenant.slug, tenant.phone, tenant.status_label || tenant.status, plan, tenantAdmins.map((admin) => admin.name).join(" ")].join(" ").toLowerCase())}"><summary><div><b>${escapeHtmlValue(tenant.name)}</b><small>${escapeHtmlValue(tenant.id)}${tenant.slug ? " | /store/" + escapeHtmlValue(tenant.slug) : ""}</small></div><span class="badge ${dueClass}">${escapeHtmlValue(tenant.status_label || tenant.status || "active")}</span><span>${escapeHtmlValue(plan)}</span><span class="${dueClass}">${escapeHtmlValue(contract)}</span><span>${escapeHtmlValue(tenantStaffUsageText(access, tenant))} 師 / ${Number(tenant.customer_count || 0)} 會員 / ${Number(tenant.booking_count || 0)} 約</span><div class="tenant-actions"><button class="mini" type="button" data-action="copy-store-url" data-slug="${escapeAttrValue(tenant.slug || "")}" title="${escapeAttrValue(publicUrl(publicPath))}" aria-label="複製店家網址">複製網址</button><button class="mini" type="button" data-action="open-store" data-slug="${escapeAttrValue(tenant.slug || "")}">開啟店家</button><button class="mini primary-action" type="button" data-action="manage-store" data-tenant-id="${escapeAttrValue(tenant.id)}">管理</button></div></summary><div class="tenant-detail"><div><h3>營運狀態</h3><div class="checks">${setupItems}</div><p>公開網址：<a href="${escapeAttrValue(publicPath)}" target="_blank">${escapeHtmlValue(publicPath)}</a></p><p>預約連結：<input readonly value="${escapeAttrValue(publicUrl(publicPath))}" onclick="this.select()"></p><p>預約狀態：${access.canAcceptBookings ? "可接受新預約" : escapeHtmlValue(tenantAccessMessage(access))}</p><p>人員方案檢查：${escapeHtmlValue(staffPlanAudit)}</p>${access.staffSelectionRequired ? `<p class="danger">人員待選擇：請進入店家後台選擇可繼續接新預約的服務人員。</p>` : access.overLimit ? `<p class="danger">服務人員已超過方案上限，目前只有已選定人員可接新預約。</p>` : ""}</div><div><h3>Admin</h3>${tenantAdmins.map((admin) => `<p><b>${escapeHtmlValue(admin.name)}</b> <span class="muted">${escapeHtmlValue(admin.role)} | ${escapeHtmlValue(admin.phone || admin.email || "-")}</span></p>`).join("") || "<p>尚無 Admin</p>"}</div></div></details>`;
